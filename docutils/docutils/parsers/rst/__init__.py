...
class Parser(docutils.parsers.Parser):
    ...
    arguments_spec = docutils.parsers.Parser.arguments_spec + (
        {'title': 'reStructuredText Parser Options',
         'description': None,
         'arguments': (
             {'flags': ('--pep-references',),
              "dest": "pep_references",
              "default": None,
              'help': 'Recognize and link to standalone PEP references (like "PEP 258").',
              'action': 'store_true'},
             {'flags': ('--pep-base-url',),
              'help': 'Base URL for PEP references (default "https://www.python.org/dev/peps/").',
              "dest": "pep_base_url",
              "default": "https://www.python.org/dev/peps/",
              'metavar': '<URL>',
              "type": frontend.validate_url_trailing_slash},
             {'flags': ('--pep-file-url-template',),
              'help': 'Template for PEP file part of URL. (default "pep-%04d")',
              "dest": "pep_file_url_template",
              "default": "pep-%04d",
              'metavar': '<URL>'},
             {'flags': ('--rfc-references',),
              'help': 'Recognize and link to standalone RFC references (like "RFC 822").',
              "dest": "rfc_references",
              "default": None,
              'action': 'store_true'},
             {'flags': ('--rfc-base-url',),
              'help': 'Base URL for RFC references (default "https://tools.ietf.org/html/").',
              "dest": "rfc_base_url",
              "default": "https://tools.ietf.org/html/",
              'metavar': '<URL>',
              "type": frontend.validate_url_trailing_slash},
             {'flags': ('--tab-width',),
              'help': 'Set number of spaces for tab expansion (default 8).',
              "dest": "tab_width",
              "default": 8,
              'metavar': '<width>',
              'type': frontend.validate_nonnegative_int},
             {'flags': ('--trim-footnote-reference-space',),
              'help': 'Remove spaces before footnote references.',
              "dest": "trim_footnote_reference_space",
              "default": None,
              'action': 'store_true'},
             {'flags': ('--leave-footnote-reference-space',),
              'help': 'Leave spaces before footnote references.',
              "dest": "trim_footnote_reference_space",
              'action': 'store_false',},
             {'flags': ('--syntax-highlight',),
              'help': 'Token name set for parsing code with Pygments: one of "long", "short", or "none" (no parsing). Default is "long".',
              "dest": "syntax_highlight",
              "default": "long",
              'choices': ('long', 'short', 'none'),
              'metavar': '<format>'},
             {'flags': ('--smart-quotes',),
              'help': 'Change straight quotation marks to typographic form: one of "yes", "no", "alt[ernative]" (default "no").',
              "dest": "smart_quotes",
              "default": False,
              'metavar': '<yes/no/alt>',
              'type': frontend.validate_ternary},
             {'flags': ('--smartquotes-locales',),
              'help': 'Characters to use as "smart quotes" for <language>. ',
              "dest": "smartquotes_locales",
              "default": None,
              'metavar': '<language:quotes[,language:quotes,...]>',
              'action': 'append',
              'type': frontend.validate_smartquotes_locales},
             {'flags': ('--word-level-inline-markup',),
              'help': 'Inline markup recognized at word boundaries only (adjacent to punctuation or whitespace). Force character-level inline markup recognition with "\\ " (backslash + space). Default.',
              "dest": "character_level_inline_markup",
              "default": False,
              'action': 'store_false'},
             {'flags': ('--character-level-inline-markup',),
              'help': 'Inline markup recognized anywhere, regardless of surrounding characters. Backslash-escapes must be used to avoid unwanted markup recognition. Useful for East Asian languages. Experimental.',
              "dest": "character_level_inline_markup",
              'action': 'store_true'}
         )},
    )
