...
class Writer(writers.Writer):
    ...
    arguments_spec = (
        {'title': 'HTML Writer Options',
         'description': None,
         'arguments': (
             {'flags': ('--template',),
              'help': 'Specify the template file (UTF-8 encoded). (default: writer dependent)',
              "dest": "template",
              "default": None,
              'metavar': '<file>'},
             {'flags': ('--stylesheet',),
              'help': 'Comma separated list of stylesheet URLs. Overrides previous --stylesheet and --stylesheet-path settings.',
              "dest": "stylesheet",
              "default": None,
              'metavar': '<URL[,URL,...]>',
             ...
             {'flags': ('--stylesheet-path',),
              'help': 'Comma separated list of stylesheet paths. Relative paths are expanded if a matching file is found in the --stylesheet-dirs. With --link-stylesheet, the path is rewritten relative to the output HTML file. (default: writer dependent)',
             ...
             {'flags': ('--stylesheet-dirs',),
              'help': 'Comma-separated list of directories where stylesheets are found. Used by --stylesheet-path when expanding relative path arguments. (default: writer dependent)',
              "dest": "stylesheet_dirs",
              "default": None,
              'metavar': '<dir[,dir,...]>',
              "type": frontend.validate_comma_separated_list},
             {'flags': ('--embed-stylesheet',),
              'help': 'Embed the stylesheet(s) in the output HTML file.  The stylesheet files must be accessible during processing. (default)',
              "dest": "embed_stylesheet",
              "default": True,
              'action': 'store_true'},
             {'flags': ('--link-stylesheet',),
              'help': 'Link to the stylesheet(s) in the output HTML file. ',
              "dest": "embed_stylesheet",
              'action': 'store_false'},
             {'flags': ('--initial-header-level',),
              'help': 'Specify the initial header level. Does not affect document title & subtitle (see --no-doc-title).(default: writer dependent).',
              "dest": "initial_header_level",
              "default": "2",
              'choices': ('1', '2', '3', '4', '5', '6'),
              'metavar': '<level>'},
             {'flags': ('--footnote-references',),
              'help': 'Format for footnote references: one of "superscript" or "brackets". (default: "brackets")',
             ...
             {'flags': ('--attribution',),
              ...
              'metavar': '<format>'},
             {'flags': ('--compact-lists',),
              'help': 'Remove extra vertical whitespace between items of "simple" bullet lists and enumerated lists. (default)',
              "dest": "compact_lists",
              "default": True,
              'action': frontend._BooleanOptionalAction},
             {'flags': ('--compact-field-lists',),
              'help': 'Remove extra vertical whitespace between items of simple field lists. (default)',
              "dest": "compact_field_lists",
              "default": True,
              'action': frontend._BooleanOptionalAction},
             {'flags': ('--table-style',),
              'help': 'Added to standard table classes. Defined styles: borderless, booktabs, align-left, align-center, align-right, colwidths-auto, colwidths-grid.',
              "dest": "table_style",
              "default": ""},
             {'flags': ('--math-output',),
              'help': 'Math output format (one of "MathML", "HTML", "MathJax", or "LaTeX") and option(s). (default: "HTML math.css")',
              "dest": "math_output",
              "default": 'HTML math.css'},
             {'flags': ('--xml-declaration',),
              'help': 'Prepend an XML declaration. ',
              "dest": "xml_declaration",
              "default": False,
              'action': frontend._BooleanOptionalAction},
             {'flags': ('--cloak-email-addresses',),
              'help': 'Obfuscate email addresses to confuse harvesters while still keeping email links usable with standards-compliant browsers.',
